...
def _zigzag_signed(values: np.ndarray, bits: int) -> np.ndarray:
    ...
    return (((values << 1) ^ (values >> 63)) & ((1 << bits) - 1)).astype(np.int32)
...
def encode_compact_carrier(
    ...
    coefficient_scales = np.asarray(coefficient_scales, dtype="<f4").reshape(-1)
    ...
    ks, coefficient_payload, coefficient_bit_count = _encode_rice(encoded_coefficients)
    return b"".join(
        [
...
def decode_compact_carrier(
    ...
    magic, basis_bit_count, coefficient_bit_count = HEADER.unpack(blob[: HEADER.size])
    if magic != MAGIC:
