...
def _apply_frame0_selector(
    ...
    codes = decode_cpr1_coefficients(
        carrier_blob, frames=renderer.N, dimensions=renderer.CARRIER_DIM
    )
    scales = cpr1_coefficient_scales(carrier_blob, dimensions=renderer.CARRIER_DIM)
    output = np.memmap(
        destination,
        mode="r+",
        dtype=np.uint8,
    ...
    for mode_index, mode in enumerate(modes):
        ...
        if not frame_ids.size:
            continue
        if mode.kind not in (COEFFICIENT, COEFFICIENT_PAIR):
            output[2 * frame_ids] = apply_pixel_mode(
                np.asarray(output[2 * frame_ids]).copy(), mode
            )
            continue
        delta_codes = np.zeros(renderer.CARRIER_DIM, dtype=np.int32)
        delta_codes[mode.a] = mode.b
        if mode.kind == COEFFICIENT_PAIR:
            delta_codes[mode.c] = mode.d
        updated_codes = codes[frame_ids] + delta_codes[None]
        if np.any(updated_codes < -2048) or np.any(updated_codes > 2047):
            raise InflationError("selector coefficient edit exceeds int12 bounds")
        if normalized_basis is None:
            normalized_basis = renderer.normalized_basis(basis.to(device))
        coefficient_delta = torch.from_numpy(
            (delta_codes.astype(np.float32) * scales).copy()
        ).to(device)
        for start in range(0, frame_ids.size, 64):
            batch_ids = frame_ids[start : start + 64]
            coefficient = coefficients[batch_ids].to(device) + coefficient_delta
            with torch.inference_mode():
                carrier = torch.einsum(
                    "bk,kchw->bchw", coefficient, normalized_basis
                ) / np.sqrt(renderer.CARRIER_DIM)
                slave = (
            ...
            output[2 * batch_ids] = (
                slave.to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
            )
    output.flush()
    return {
...
def inflate_archive(
    ...
    *,
    ...
    legacy_width_marker = bytes(40_252)
    semantic_pose = (
        struct.pack("<II", len(legacy_width_marker), len(canonical_carrier))
        + legacy_width_marker
        + canonical_carrier
    )
