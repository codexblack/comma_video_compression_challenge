"""Canonical little-endian signed fixed-width packing."""
...
def pack_signed(codes: Iterable[int], bits: int) -> bytes:
    """Pack two's-complement codes, least-significant bit first."""
    minimum, maximum = _bounds(bits)
    acc = 0
    used = 0
    result = bytearray()
    mask = (1 << bits) - 1
    for raw in codes:
        ...
        if not minimum <= code <= maximum:
            raise BitPackingError(f"{code} does not fit signed int{bits}")
        acc |= (code & mask) << used
        used += bits
        while used >= 8:
